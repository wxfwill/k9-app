...
class RoundComponent extends Component{
  constructor(props){
    ...
    this.state={
      roudData:[],
      user:user,
      statusText: { // status 0 未开始 1 进行中 2 停止 3 over
        0: '开始巡逻',
        1: '巡逻进行中',
        2: '巡逻暂停',
        3: '任务结束'
  	...
  	return(
        ...
        <div className="round-header">
         ...
         <WingBlank className="duty-list">
            <List>
            {
              roudData.length>=1?roudData&&roudData.map((item,index)=>{
                return(
                  <Item
                    className="duty-item"
                    extra={<p className={classnames('mark',{active:item.status==0})} onClick={item.status !== 3?() => this.handleRound(item):null}><span>{statusText[item.status]}</span></p>}
                    multipleLine
                    key={index}
                  >
                    {item.taskName} <Brief>{new Date(item.startTime).toLocaleString()}{item.patrolsPlace}</Brief>
                  </Item>
                )
              }):<WingBlank className="normal-data" size="sm">暂无数据...</WingBlank>
            }
