...
class AddComponent extends Component{
	...
	requestCombatType(){
		commonJs.ajaxPost('/api/basicData/combatType', {}, result => {
			if(result.code == 0){
	...
	handleOk(){
  	...
  	handleSubmit(){
		...
		data.id = this.state.id;
  		commonJs.ajaxPost('/api/cmdMonitor/publishEmergencyDeploymentPlan', data, (result) => {
  			if(result.code == 0){
  				this.isRequest = false;
				util.toast('发布成功！')
			...
			<div className="add-content">
				...
				<div className="list-box">
					<List className="list">
					    ...
					    >
					        <Item arrow="horizontal"><i className="tips">*</i>选择日期</Item>
					    </DatePicker>
				    </List>
				    <List className="list">
				        <InputItem
				    ...
				    <div className={`list pointer-list ${disabled ? 'list-disable' : ''}`}>
				    	<div className="name"><i className="tips">*</i>作战区域</div>
				    	<div className="cont">
				    	{
				    		location ? location : ''
				    	}
				    	</div>
				    	<div className="icon" onClick={()=>this.choosePoint()}>{disabled ? '查看' : '添加' }</div>
				    </div>
				    <div className={`list pointer-list ${disabled ? 'list-disable' : ''}`}>
				    	<div className="name"><i className="tips">*</i>作战类型</div>
				    	<div className="cont">
				    	{
				    		combatTypeName ? combatTypeName : ''
				    	}
				    	</div>
				    	{
				    		disabled ? '' :
				    		<div className="icon" onClick={()=>this.chooseType()}>选择</div>
				    	}
				    </div>
				    <Choose 
				    	title="作战人员"
				    ...
				    <List className="list">
				    	<TextareaItem
				            ...
				            title={<span><i className="tips">*</i>作战内容</span>}
				            disabled={disabled}
				            clear
				            placeholder="作战内容"
				            data-seed="logId"
				...
				{ !disabled ? 
					<div className="btn-box">
						<button className="clear" onClick={() => this.clearAll()} style={{display:'none'}}>清空</button>
						<button className="publish" onClick={() => this.handleSubmit()}>发布</button>
					</div>
					:
					null
				}
				...
				<div className="btn-box only-btn">
					<button className="stop" onClick={this.showStop}>终止任务</button>
				</div>
				: null
				}
				{this.state.isMap ? 
		        ...
		        >
		        	<div className="pick-view-btn">
		        		<span className="cancel" onClick={() => this.combatCancel()}>取消</span>
		        		<span className="sure" onClick={() => this.combatOk()}>确定</span>
		        		<div>请选择作战类型</div>
		        	</div>
		          	<PickerView
		          		cols={1}
