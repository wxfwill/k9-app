  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (sessionStorage.getItem('socketData')) {
        const socketData = JSON.parse(sessionStorage.getItem('socketData'));
        console.log(JSON.parse(sessionStorage.getItem('socketData')), '====================');
        if (socketData.code == 0) {
          this.setState({
            newsNum: socketData.data.total,
          });
          // 消息列表根据返回的数据和本地json更新列表
          let arrList = [];
          this.state.newList.map((list) => {
            socketData.data.items.map((item) => {
