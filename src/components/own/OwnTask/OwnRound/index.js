...
class OwnRoundComponent extends Component{
    constructor(props){
        super(props);
    ...
    goDetail = (item) => {
      ...
      history.push({pathname:'/ownround/map', query:item.taskDetailId })
    }
    getContent(data, callback){
        let _this = this;
        commonJs.ajaxPost('/api/cmdMonitor/myPatrolsTask', {startDate: data}, function(result){
          if(result.code == 0){
...
export default withRouter(OwnRoundComponent);



// WEBPACK FOOTER //
// ./src/components/own/OwnTask/OwnRound/index.js