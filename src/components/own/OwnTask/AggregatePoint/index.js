...
class OwnEmDepComponent extends Component{
    ...
    goDetail = (item) => {
      ...
      history.push({pathname:'/aggpoint/map', query:item.id })
    }
    getContent(data, callback){
        let _this = this;
        commonJs.ajaxPost('/api/cmdMonitor/myAssembleTask', {assembleDate: data}, function(result){
          if(result.code == 0){
    ...
    render(){
        ...
        const row = (rowData, sectionID, rowID) => {
            ...
            let st = stateArr[obj.taskStatus];
            return (
              <div
                key={rowID}
                className="list-item"
              ...
              >
                  ...
                  <div className="cont">
                    ...
                    <div>
                      <span>集合日期：</span>
                      <span>{obj.assembleTime ? moment(obj.assembleTime).format('YYYY-MM-DD') : '--'}</span>
                    </div>
                  </div>
              </div>
            );
...
export default withRouter(OwnEmDepComponent);



// WEBPACK FOOTER //
// ./src/components/own/OwnTask/AggregatePoint/index.js