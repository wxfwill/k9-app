...
class DogManage extends Component {
  constructor(props) {
    super(props);
  ...
  handleSearchList = (per) => {
    ...
    React.$ajax.own.myTreatmentRecord(per).then((res) => {
      if (res.code == 0) {
            ...
            thumb={
              <span
                style={{
                  ...
                  borderRadius: '50%',
                  overflow: 'hidden',
                  background: `url(${require('images/own/own-dog-report.svg')}) left top no-repeat`,
  ...
  render() {
    ...
    return (
      <div className="roll-call-wrap">
        <Header pointer title="上报列表" myRef={this.headerHeight} />
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(DogManage));
