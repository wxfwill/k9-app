...
class OwnLevelComponent extends Component{
    ...
    goDetail = (item) => {
      ...
      console.log(history);
      history.push({pathname:'/own/call/CallDetails', query:item.id })
      console.log(history);
    }
    ...
    getContent(data, callback,flag){
        let _this = this;
        commonJs.ajaxPost('/api/attendance/listPage', {qryDateStr: data,currPage:flag? 1 :_this.state.currPage,pageSize:5}, function(result){
          if(result.code == 0){
