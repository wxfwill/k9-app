...
class Report extends Component {
  ...
  chooseNameCont = (keys, data) => {
    ...
    data.map((item) => {
      arr.push(
        <Item key={item.id} data-seed={keys} value={item.commentName}>
          {item.commentName}
        </Item>
      );
    });
  ...
  render() {
    ...
    return (
      <div className="report-box">
        <Button type="primary" inline size="small" onClick={this.onOpen}>
          {reportState[stateNum]}
        </Button>
        <Modal className="report-modal" popup animationType="slide-up" visible={modalShow} onClose={this.onClose}>
            ...
            {reportDetail.status == 0 ? (
              <List renderHeader={() => <div>任务上报</div>}>
                {type != 6 ? (
                  <div>
                    {' '}
                    <InputItem type="text" placeholder="请输入查缉区域" onChange={this.placeChange}>
                      查缉区域
                    </InputItem>
                    <InputItem onChange={this.substationChange} placeholder="请输入分局名称">
                      分局名称
                    </InputItem>
                  </div>
                ) : null}
                {type != 6 ? (
                  <div className="add-box">
                    ...
                    <div className="btn" onClick={this.addObject}>
                      添加
                    </div>
                  </div>
                ) : null}
              ...
              <List renderHeader={() => <div>上报详情</div>}>
                ...
                {type != 6 ? (
                  <div>
                    <InputItem disabled={true} defaultValue={reportDetail.palce || ''}>
                      查缉区域
                    </InputItem>
                    <InputItem disabled={true} defaultValue={reportDetail.substation || ''}>
                      分局名称
                    </InputItem>
                    {reportDetail.commentList &&
                      reportDetail.commentList.map((item, index) => (
                        <InputItem key={'detail' + index} disabled={true} defaultValue={item.number}>
                          {item.commentName}
                        </InputItem>
                      ))}
                  </div>
          ...
          <div className="modal-footer">
            ...
            {reportDetail.status == 0 && (
              <div className="sure" onClick={this.sendReportData}>
                确定
              </div>
            )}
          </div>
