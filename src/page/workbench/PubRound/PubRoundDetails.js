...
class PubRoundMap extends Component{
  ...
  componentDidMount() {
   ...
   this.timer2 = setInterval(this.getUserLocation,4000) // 获取用户的app位置信息
     ...
     if(!this.props.taskInfo || this.props.taskInfo.saveStatus==0){
  ...
  getTaskInfo(){
   ...
   if(Number(this.props.location.query)>0){
        Ajax.post('/api/dailyPatrols/getDailyPatrolsById', {
        id: this.props.location.query
    }, (res) => {
        if(res.code == 0){
            let userNames='';
            for(var p in res.data.userMap){
                userNames+=res.data.userMap[p]+'，';
             }
             res.data.userNameList=userNames.substring(0, userNames.lastIndexOf('，'));
            this.setState({
                taskInfo:res.data
            });
            let path=[];
            if(res.data && res.data.drawShapeDTO!==''){
                res.data.drawShapeDTO.latLngArr.map((item) => {
  ...
  //巡逻人员列表
  getRoundList(id){
    Ajax.post('/api/dailyPatrols/getUserByTaskId', {
        taskId:id
    }, (res) => {
        if(res.code == 0){
            res.data.map((item)=>{
  ...
  onSearch =(value)=>{
    ...
    fetchTrochoid=(params)=>{
      ...
      getNowTrochoid=(data)=>{
        ...
        Ajax.post('/api/cmdMonitor/showAppTrochoid', {lastPointTime:trochoiInfo.lastPointTime,taskType:trochoiInfo?trochoiInfo.taskType:'',taskDetailId:trochoiInfo?trochoiInfo.taskDetailId:''}, (res) => {
      ...
      drawTrace=(gpsData,flag,strokeColor)=>{ //绘制轨迹
        ...
        if(gpsData.length>0){
          ...
          if(typeof flag!=undefined && flag==1){  //定位到轨迹位置
      ...
      showStop = (id) => {
        const alertStop = alert('终止任务', '确定终止此任务吗?', [
  ...
  //终止任务
  stopTask = (id)=>{
    Ajax.post('/api/cmdMonitor/stopPatrols', {
           id: id
         }, (res) => {
             if(res.code == 0){
  ...
  render(){
    ...
    const tabs = [
  	...
  	return(
  		<div className="map">
        <div id="container" className="container" style={{top:0,zIndex:10}}>
        <div className="mapPanel" >
           <div className="content" >
               <div className="iconDiv" onClick={() => this.showDetail(isShowDetail)}>
                    <Icon color="#666666" type={ isShowDetail ? 'down' : 'up'} />
                </div>
           {/*     <div className={ isShowDetail ? '':'rhide'}>*/}
                <Tabs tabs={tabs}
                    initalPage={'t2'}
                >
                    <div className={ isShowDetail ? 'map-infor':'map-infor rhide'}>
                        {userList&&userList.length>0?<div> <WhiteSpace size="lg" />
                            <Flex>
                            <Flex.Item><div style={{textAlign:'center'}}>颜色</div></Flex.Item>
                            <Flex.Item><div style={{textAlign:'center'}}>姓名</div></Flex.Item>
                            <Flex.Item><div style={{textAlign:'center'}}>犬名</div></Flex.Item>
                            <Flex.Item><div style={{textAlign:'center'}}>状态</div></Flex.Item>
                                </Flex></div>:null}
                            {userList&&userList.length>0?userList.map((item) =><div ><WhiteSpace size="lg" /><Flex onClick={()=>this.handonRowClick(item)}>
                                <Flex.Item><div style={{textAlign:'center'}}><Tag small style={{background: item.color?item.color:'',color: '#fff'}} ></Tag></div></Flex.Item>
                                <Flex.Item><div style={{textAlign:'center'}}>{item.userName?item.userName:'--'}</div></Flex.Item>
                                <Flex.Item><div style={{textAlign:'center'}}>{item.dogName?item.dogName:'--'}</div></Flex.Item>
                                <Flex.Item><div style={{textAlign:'center'}}>{this.getStateText(item.status)}</div></Flex.Item>
                                </Flex>
                                </div>):<div style={{ textAlign: 'center',paddingTop:'12px',paddingBottom:'12px'}}>暂无数据</div>}
                    </div>
                    <div className={ isShowDetail ? 'map-infor':'map-infor rhide'}>
                        <section>
                            <span>任务名称：</span>
                        ...
                        <section>
                            <span>巡逻地点：</span>
                            <div>{taskInfo&&(taskInfo.patrolsLocation||'--')}</div>
                        </section>
                        <section>
                            <span>巡逻人员：</span>
                            <div>{taskInfo&&(taskInfo.userNameList||'--')}</div>
                        </section>
                        <section>
                            <span>任务内容：</span>
                  ...
                  {/*taskInfo&&taskInfo.saveStatus==1&&taskInfo.taskStatus<2 ?
                      ...
                      >终止任务</Button></span>:null*/}
                       {taskInfo&&taskInfo.saveStatus==1&&taskInfo.taskStatus==0 ?
                        ...
                        size="small" 
                        type="primary"
                        onClick={()=>this.startOrEndPatral(taskInfo.id,0)}
                      >开始任务</Button></span>:null}
                       {taskInfo&&taskInfo.saveStatus==1&&taskInfo.taskStatus==1 ?
                        ...
                        type="primary"
                        className="stop-btn"
                        onClick={()=>this.startOrEndPatral(taskInfo.id,1)}
                      >结束任务</Button></span>:null}
                    <span  onClick={this.handleShow.bind(this)}>
                       <Button   type="primary" inline size="small">返回</Button>
                    </span>
              </section>
            </div>
          </div>
...
// WEBPACK FOOTER //
// ./src/redux/containers/publish/PubRound/PubRoundDetails.js