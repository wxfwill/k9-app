...
class PubRoundMap extends Component{
  ...
  componentDidMount() {
   ...
   this.timer2 = setInterval(this.getUserLocation,4000) // 获取用户的app位置信息
    ...
    let path=[];
    if(this.props.taskInfo && this.props.taskInfo.drawShapeDTO!==''){
      this.props.taskInfo.drawShapeDTO.latLngArr.map((item) => {
  ...
  onConfirm(){
    if(this.TMap.isClose){
      ...
      let address = this.TMap.resolveLatLng(drawShapeDTO.coord, function (address) {
        _this.props.onMapData({ patrolsLocation: address, drawShapeDTO: drawShapeDTO });
      });
      this.props.onCancel();
    }else{
      toast("请选择目标区域！");
  	...
  	return(
        ...
        <div className="mapPanel" >
           ...
           <section>
              ...
              {this.props.taskInfo && this.props.taskInfo.saveStatus==1 ? 
                <section>
...
// WEBPACK FOOTER //
// ./src/redux/containers/publish/PubRound/PubRoundMap.js