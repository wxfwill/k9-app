...
class ApprovalDetailsComponent extends Component {
  ...
  handleSubmit(id, flag, opinion) {
    ...
    let _this = this;
    const dataObj = { id: id, verify: flag, opinion: opinion };
    React.$ajax.publish.verifyLeaveApply(dataObj).then((res) => {
      if (res && res.code == 0) {
  ...
  render() {
    ...
    const separator = () => <div>fsfsaf</div>;
    return (
      <div className="OwnLevel">
        ...
        <div className="foot">
          <WingBlank>
            {disabled && approvalInfo && approvalInfo.status == 0 ? (
              <div>
                <Button
                  className="yes"
                  onClick={() =>
