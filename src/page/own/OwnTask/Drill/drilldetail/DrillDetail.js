...
class DrillDetail extends Component{
  ...
  componentDidMount() {
    ...
    if(this.props.history.location.query){
      Ajax.post('/api/train/trainAppDetail', {
        id: this.props.history.location.query
      }, (res) => {
          if(res.code == 0){
            const dogList = res.data.dogs && res.data.dogs.map((t) =>  {
    ...
    const { subjectDetails, allPeoples, peoShow, trackState, canStart } = this.state;
  	return(
  		<div className="Drill">
          {
            ...
            <Header title={`${taskName}`} pointer="pointer" isSet="轨迹分析" handleShow={() => this.handleShow(this.state.drillInfo.id)} />
            :
            <Header title={`${taskName}`} pointer="pointer" />
          }
        <List className="list">
          <TextareaItem title="训练场地:" disabled="true" placeholder="" value={this.state.drillInfo.placeName}></TextareaItem> 
        </List>
    
        <List className="list">
            <TextareaItem title="训练科目:" disabled="true" placeholder="" value={this.state.drillInfo.subjectName}></TextareaItem>
        </List>
        <List className="list">
            <TextareaItem title="训练说明:" autoHeight  disabled="true" placeholder="" value={this.state.drillInfo.trainRemark}></TextareaItem>
        </List>
        <List className="list">
            {this.state.drillInfo.status >=1 ?
              <TextareaItem title="犬名:" disabled="true" placeholder="" value={this.state.drillInfo.dogName ? this.state.drillInfo.dogName : `无`}></TextareaItem>
              :
              <Picker data={this.state.dogs} cols={1} {...getFieldProps('district3')}   className="forss"
        ...
        <List className="list">
            {this.state.drillInfo.status !=0 || this.state.drillInfo.isTrace ==1 ?
              <TextareaItem title="训练专项:" disabled="true" placeholder="" value={this.state.drillInfo.subjectDetailName}></TextareaItem>
              :
              <Picker 
                data={subjectDetails}
                className="forss"
        ...
        {this.state.drillInfo.isTrace == 1?
          <List className="list">
              <TextareaItem title="追踪对象:" disabled="true" placeholder="" value={this.state.drillInfo.traceTargetUserName}></TextareaItem>
          </List>
              :
              null
        }
        ...
        }
        {
          ...
          <List className="list state-list">
            <List.Item extra={<i className={trackState.class}>{trackState.name}</i>}>{this.state.drillInfo.traceTargetUserName}</List.Item>
          </List>
          :
          null
        }
        {canStart ? 
          <div style={{textAlign:'right', padding:'6px 15px'}}>
            <Button type="primary"   onClick={this.surePeople} inline size="small">确认追踪对象</Button>
          </div>
          :
          null
        }
        ...
        */}
         ...
         <div className="handle">
          {this.state.drillInfo.status==0 ?
            <Button type="primary"   onClick={this.trainStartOrEnd} inline size="small">开始训练</Button> : null
          }
          {this.state.drillInfo.status==1 ?
            <Button type="primary"   onClick={this.trainStartOrEnd} inline size="small">结束训练</Button>:null
          }
          {this.state.drillInfo.status==2 ?
            <Button className="end"   inline >完成</Button> : null
          }
