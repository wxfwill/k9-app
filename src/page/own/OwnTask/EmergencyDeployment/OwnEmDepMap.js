...
class OwnEmDepMAP extends Component{
  ...
  componentDidMount() {
    if(this.props.history.location.query) { // 绘制目标点和区域
      ...
      this.getDetail(taskId, (data) => {
        ...
        let area = data.drawShapeDTO.coord;
        let radius = data.drawShapeDTO.bdRadius;
        let options = {
          lat: 22.53103578227407,
          lng: 113.95115796895573
        }
        if(area) {
            options = Object.assign(options, area);
            this.TMap = new tMap(options);
            this.polyline = this.TMap.targetCircle(area, radius, '#2691EA');
            this.referencePoint = data.drawShapeDTO.coord;
  ...
  getDetail = (taskId, fn) => {
    Ajax.post('/api/cmdMonitor/emergencyTaskAppDetail', {id: taskId}, (result) => {
      if(result.code == '0') {
        fn(result.data)
      }
    })
  ...
  confirmDog = (patral)=>{
   ...
   /* if(value==null){
    ...
    sendData.dogId = value;
    query.dogName = value;
    const requstUri = '/api/cmdMonitor/startEmergency';
    Ajax.post(requstUri,sendData,(data) => {
     if(data.code == '0') {
      _this.setState({
        detailData:query
  ...
  render(){
    ...
    const obj = this.state.detailData;
  	return(
        ...
        <Header title={obj ? '紧急调配详情' : '加载中'} closeTask={this.closeTask.bind(this)} updateTaskStatus={this.updateTaskStatus.bind(this)} pointer="pointer"/>
        	...
        	{ obj ?
          	<div className="roundPanel">
            	<div className="content">
                    ...
                    <section>
                        <span>任务类型：</span>
                        <div>{obj.combatTypeName}</div>
                    </section>
	                	<section>
	                  		<span>作战时间：</span>
	                  		<div>{moment(obj.planStartDate).format('YYYY-MM-DD')}</div>
	                	</section>
	                	<section>
	                  		<span>犬只：</span>
	              	...
	              	<header>
		                <p>
                      开始时间：{moment(obj.planStartDate).format('YYYY-MM-DD')}
                    </p>
	              	</header>
...
// WEBPACK FOOTER //
// ./src/components/own/OwnTask/EmergencyDeployment/OwnEmDepMap.js