...
class EmergencyDeployment extends Component {
  constructor(props) {
    super(props);
  ...
  handleSearchList = (per) => {
    React.$ajax.own.myUrgonTaskList(per).then((res) => {
      if (res.code == 0) {
  ...
  renderRow = (rowData) => {
    ...
    return (
      item && (
        <List className="new-list-type" key={item.taskId} onClick={this.handleNoNews.bind(this, item)}>
          <Item
            extra={
              <div
                className="finsh"
            ...
            thumb={
              <span
                style={{
                  ...
                  overflow: 'hidden',
                  background: `url(${require('images/own/tasktab/active-jing.svg')}) left top no-repeat`,
                  backgroundSize: '100% 100%',
          ...
          >
            ...
            <div className="new-desc">
              ...
              {item.planUserNames && item.planUserNames.join(' 、')}
            </div>
          </Item>
          {
            <div
              className="task-btn"
              onClick={() => {
                this.props.history.push(`/own/EmergencyDeploymentDetal?id=${JSON.stringify(item)}`);
              }}
            >
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(EmergencyDeployment));
