...
function socket(commit, actions) {
   ...
   * @param value 数据处理
   */
  this.$soctket_init = function (callback) {
    const _this = this;
    if (_this.closeWs) {
   ...
   * @param value 数据处理
   */
  this.$soctket_emit = function (value, callback) {
    const _this = this;
    const poll = function () {
  ...
  this.$soctket_subscribe = function () {
    const _this = this;
