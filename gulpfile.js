...
function showAddress(env) {
  let url = null;
  if (env == 'dev') {
    url = 'http://' + config.ssh.host + ':8010' + '/k9-app';
  } else if (env == 'test') {
    url = 'http://' + config.ssh.host + ':8000' + '/k9-app';
  } else if (env == 'outerDev') {
    url = 'http://test8.hua-cloud.net:5519/k9-app';
  } else if (env == 'outerTest') {
    url = 'http://test8.hua-cloud.net:5518/k9-app';
  }
  return url;
}
...
gulp.task(
  ...
  gulp.series('execSSHDelete', 'publish', 'execSSHBackup', (done) => {
    ...
    console.log(showAddress(config.env));
    // Did you forget to signal async completion? 报错后需要调用done,以结束task
    done(); // 在不使用文件流的情况下，向task的函数里传入一个名叫done的回调函数，以结束task
  })
);
